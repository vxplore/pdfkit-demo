...
import { TableDataType } from './pdf3.controller';
...
export class Pdf3Service {
  ...
  public headerHeight = this.m * 6;

  generateInvoiceData(doc, tableData: TableDataType) {
    const m = this.m;
    doc
      .font('Helvetica-Bold')
      .fontSize(14)
    ...
    doc
      ...
      .lineTo(doc.page.width / 2.5, m * 14.5)
      .stroke();
  ...
  generateTable(
    ...
    let p = w;
    w = p + nameWidth;
    doc.text('Name of Product/Service', p - m / 2, ts + m, {
      align: 'center',
      width: w,
    });
    doc
    ...
    p = w;
    w = p + taxableWidth;
    doc.text('Taxable value', p + m / 2, ts + m, {
      align: 'center',
      width: taxableWidth,
    });
    doc
